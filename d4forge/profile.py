...
class ResolvedProfile:
    ...
    base: EnchantProfile
    client: Rect
    _sx: float = field(init=False)
    _sy: float = field(init=False)

    def __post_init__(self) -> None:
        self._sx = self.client.w / REFERENCE_WIDTH
        self._sy = self.client.h / REFERENCE_HEIGHT

    def rect(self, ref: Rect) -> Rect:
        """Converte um Rect de referencia para coordenadas de tela."""
        return Rect(
            self.client.x + int(round(ref.x * self._sx)),
            self.client.y + int(round(ref.y * self._sy)),
            max(1, int(round(ref.w * self._sx))),
            max(1, int(round(ref.h * self._sy))),
        )

    def rects(self, refs) -> list[Rect]:
        return [self.rect(r) for r in refs]
